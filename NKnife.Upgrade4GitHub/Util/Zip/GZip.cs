                ...
                try
                {
                    ...
                    foreach (FileInfo fi in files)
                    {
                        try
                        {
                            ...
                            string fileModDate = fi.LastWriteTimeUtc.ToString(CultureInfo.InvariantCulture);
                            gzf.ModifiedDate = fi.LastWriteTimeUtc;
                            gzf.Length = buffer.Length;

                            string fileHeader = $"{fileIndex}|{vpSourceFile}|{fileModDate}|{buffer.Length}\n";
